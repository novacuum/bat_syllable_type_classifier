...
"""Experiments on the SRNE-P dataset using Neural Networks and repeated training 
...
def run():
    ...
    for dataset, base in [srnep_c6(), srnep_c2()]:
        ...
        data = {
            ...
            f'raw{IMG_PEAK_XPPS_3}': base.create_spectrogram(height=img_height,
                                                             x_pixels_per_sec=IMG_PEAK_XPPS_3).img_features(),
        ...
        for f, t in data.items():
            common_params = dict(
                ...
                model_suffix=f'_{f}_multi_e{max_epochs}'
            )
            lstm_params = dict(
                **common_params,
                lstm_neurons=256,
