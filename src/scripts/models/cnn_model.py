...
def create_cnn_model(input_shape, num_classes, learning_rate):
    ...
    if can_conv_pool_2d(dims):
        model.add(Conv2D(64, (3, 3), activation='relu'))
        model.add(MaxPooling2D(pool_size=(2, 2)))
        conv_pool_2d(dims)  # Now is 64x32

    if can_conv_pool_2d(dims):
        model.add(Conv2D(128, (3, 3), activation='relu'))
        model.add(MaxPooling2D(pool_size=(2, 2)))  # Now is 32x16
        conv_pool_2d(dims)

    if can_conv_pool_2d(dims):
        model.add(Conv2D(256, (3, 3), activation='relu'))
        model.add(MaxPooling2D(pool_size=(2, 2)))  # Now is 16x8
        conv_pool_2d(dims)
...
def test_cnn_model(
    *,
    ...
    model_name = f'cnn_2d_{dataset.name}{model_suffix}'
